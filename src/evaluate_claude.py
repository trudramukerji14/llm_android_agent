...
def run_episode(episode, template):
    ...
    for obs in episode["observations"]:
        ...
        print(prompt)
        print("\n------------------------------\n")
        action, reason = query_llm(prompt, model= "claude-opus-4-20250514")  # specify claude model here
...
def main():
    ...
    Path("results_claude").mkdir(exist_ok=True) #directory for claude results

    for i, episode in enumerate(episodes):
        zero_result = run_episode(episode, zero_template)
        few_result = run_episode(episode, few_template)

        with open(f"results_claude/episode_{i+1:02}_zero.json", "w") as f:
            json.dump(zero_result, f, indent=2)
        print(f"[Claude Zero-Shot] Finished episode {i+1}")

        with open(f"results_claude/episode_{i+1:02}_few.json", "w") as f:
            json.dump(few_result, f, indent=2)
        print(f"[Claude Few-Shot]  Finished episode {i+1}")
